# Deep Q-Network (DQN) for Dynamic Prompt Optimization on Large Language Models
...
This repository contains the complete implementation of a **Reinforcement Learning** approach to **Prompt Engineering**. Instead of relying on static, human-crafted prompts, we train a Deep Q-Network (DQN) agent to dynamically select and inject prompt modifiers into a frozen Large Language Model (`TinyLlama-1.1B-Chat-v1.0`). 
...
The agent was trained for **40 epochs** (160 train samples per epoch) and evaluated on a holdout test set (40 samples). 
